...
---
이번에 사이드 프로젝트에서 Kubernetes 환경에서 CI / CD 파이프라인을 구축하게 되었습니다.
그 중 CI 파이프라인을 구축하는데 Jenkins를 사용하기로 했었습니다.
여기서 제가 GitOps 기반으로 Jenkins CI 파이프라인을 구축한 경험을 공유해 드리려고 합니다.

# Jenkins 설치

우선 Jenkins를 클러스터 내에 구동해야 했습니다.
이때 사용하는 방법으로는 Helm을 사용하기로 했습니다.
그 이유는 Jenkins와 관련된 파드, 서비스, 인그레스 등을 수동으로 배포하고 관리하기는 힘들다고 생각했기 때문입니다.
...
인그레스를 Root 권한으로 실행할 것이며 이미 존재하는 `PersistenceVolumeClaim`을 사용할 것입니다.
또한 `/jenkins`를 Prefix로 가지는 인그레스를 생성할 것이므로 이에 맞게 `jenkinsUriPrefix`도 수정해줍니다.
인그레스는 제가 사전에 배포한 `NodePort` 타입의 Nginx Ingress Controller에 설정됩니다.
...
이번 프로젝트에서 사용하는 Kubernetes는 AWS EKS(Elastic Kubernetes Service) 등의 Managed Kubernetes가 아니므로 Block Storage를 프로비저닝할 Storage Class를 사용하기는 어렵습니다.
...
spec:
  ...
  template:
    ...
    spec:
      ...
      - name: docker-secret
```
`templates`에 디플로이먼트, 서비스, 인그레스 등에 대한 Manifest들을 작성합니다.
