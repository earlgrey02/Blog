...
이는 C2 컴파일러의 더 높은 수준의 최적화 때문입니다.
그래서 애플리케이션을 시작할때는 C1 컴파일러가, 장기적으로는 C2 컴파일러를 사용하는 것이 유리합니다.
...
이때, 기존의 C1 컴파일러의 코드가 진입 불가 상태가 되어 `made not entrant`로 표시되고 컴파일된 코드를 원상태로 돌리는 역최적화가 수행되는 것을 확인할 수 있습니다.
