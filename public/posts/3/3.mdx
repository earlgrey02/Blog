...
tags: [Kotlin, Reactor]
---
이번 글은 토이 프로젝트에서 Reactor의 `buffer()`와 `concatMap()`을 통해 동기 처리를 구현하는 과정을 담은 글이다.


## 배경

토이 프로젝트로 학교 편의 앱을 개발하면서 우리 학교의 API를 사용하다 문제를 겪었다.
난 학생들에게 본인이 신청한 강의에 대한 정보를 제공하는 기능을 구현하려고 했다.
