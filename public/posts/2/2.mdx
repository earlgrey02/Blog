...
---
이번 글은 토이 프로젝트에서 Spring Batch를 통해 배치 작업을 구현하는 과정을 담은 글이다.

## 배경

토이 프로젝트로 학교 편의 앱을 개발하면서 우리 학교의 API를 사용하게 되었다.
구현할 여러 기능 중 하나인 강의 검색 기능은 학교의 강의 조회 API를 이용해 기존에 존재하는 학교 사이트의 강의 검색 기능의 속도나 편의를 개선해서 만들고자 했다.
...
난 Spring Batch로 학교 데이터베이스와 서버 데이터베이스를 동기화하는 배치 작업을 정의하기로 했다.
...
또한 배치 작업에 스케줄링을 적용하기 위해 `@EnableScheduling`을 등록한 `BatchConfiguration`을 구현했다.

## Step 구현

`Step`은 Chunk나 Tasklet을 기반으로 구현할 수 있다.
Tasklet 기반 배치는 `Step`을 하나의 메서드로 한번에 처리한다.
반면에 Chunk 기반 배치는 `Step`을 `read()`, `process()`, `write()`로 구성해 설정한 `chunk` 수만큼 데이터를 묶어 처리한다.
