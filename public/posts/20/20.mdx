...
description: Spring WebFlux에서 채팅 기능을 구현하는 방법
date: 2024-05-14
tags: [Kotlin, Spring, Spring WebFlux, Kafka, WebSocket]
---
...
```kotlin title="ExampleWebSockerHandler.kt" showLineNumbers /Mono.zip/
override fun handle(session: WebSocketSession): Mono<Void> =
    Mono.zip(
        session.receive()
            .map { ... }
...
```kotlin title="MessageWebSocketHandler.kt" showLineNumbers {12, 15, 20-26, 29-33}
@Component
