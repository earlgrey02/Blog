...
```ts title="debounce.ts" showLineNumbers {3, 5}
...
```ts title="hook.ts" showLineNumbers {12-20}
const useLike = (post: PostResponse) => {
  ...
  const [initialIsLiked, setInitialIsLiked] = useState(isLiked)
...
해당 게시글의 좋아요 개수인 `like` 및 현재 유저의 좋아요 여부인 `isLiked` 등을 포함한 Custom Hook을 구현했습니다.
`isLiked`가 처음과 달라졌는지 확인하기 위해 최초의 `isLiked`를 `initialIsLiked`에 저장했습니다.
좋아요를 클릭하면 `likeHandler`에 의해 `isLiked`의 상태가 바뀝니다.
이때, 호출되는 `useEffect()`에서 디바운싱을 통해 좋아요를 여러 번 클릭해도 마지막 좋아요 요청만 처리하도록 최적화를 수행했습니다.

<br /><br />

최종적으로 `isLiked`가 `initialIsLiked`와 다르다면 바뀐 좋아요 여부를 서버에 반영하고 `initialIsLiked`를 현재 좋아요 여부로 갱신합니다.
...
이전과 달리 많은 좋아요 요청에도 API 호출이 한 번만 발생한 것을 확인할 수 있습니다.
