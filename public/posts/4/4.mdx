...
Kubeadm 기반 클러스터에서 ETCD는 `kube-system` 네임스페이스에 Static Pod로 배포되어 있으므로 `/etc/kubernetes/manifests`에 있는 `etcd.yaml`로 ETCD에 대한 정보를 볼 수 있다.
...
내부적으로 Kubelet이 ETCD의 변경 사항을 감지하고 클러스터 내의 상태를 변경한다.
